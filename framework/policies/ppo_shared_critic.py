    ...
    def action(self, observations, **kwargs):
        ...
        for i in range(self.n_agents):
            ...
            (
                ...
                value,
                self.actor_hidden[i],
            ) = self.agent.choose_action(obs_batch, val_obs, i, self.actor_hidden[i])
...
class PPOTrainer:
    ...
    def store_memory(self, i, observations, val_observations, logprobs,action,vals,reward,done,actor_hidden):
...
class NNN(nn.Module):
    ...
    def get_action_and_value(self, x, val_x, actor, hidden_actor, action=None):
        inp_hidden_actor = torch.hstack([x, hidden_actor])
        inp_hidden_critic = torch.hstack([val_x, hidden_actor])
...
class Agent:
    ...
    # fmt:off
    def remember(self, i, observations, action_p, action, vals, val_obs, reward, done, actor_h):
        self.memory.store_memory(i, observations, val_obs, action_p, action, vals, reward, done, actor_h)
    ...
    # fmt:on
    def choose_action(self, observations, val_obs, i, actor_hidden):
        with torch.no_grad():
            (
                action,
                probs,
                ...
                value,
                new_hidden_actor,
            ) = self.ppo.get_action_and_value(observations, val_obs, i, actor_hidden)
