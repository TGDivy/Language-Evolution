...
class DDPG:
    ...
    def train(self, workers):
        ...
        if self.alg_mode == MADDPG:
            if self.discrete_action:
                target_actions = [
                    onehot_from_logits(worker.target_actor(nobs))
                    for worker, nobs in zip(workers, obs_next_n)
                ]
            else:
                target_actions = [
                    worker.target_actor(nobs)
                    for worker, nobs in zip(workers, obs_next_n)
                ]
            ...
            if self.discrete_action:
                target_actions = onehot_from_logits(self.target_actor(s2_batch))
            else:
        ...
        if self.alg_mode == MADDPG:  # Get the actions of all actors in MADDPG mode.
            for i, worker, obs in zip(range(len(workers)), workers, obs_n):
                if i == self.pos:
                    ...
                    all_pol_acs.append(onehot_from_logits(worker.actor(obs)))
                else:
